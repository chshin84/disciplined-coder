...
## 무엇이 좋은가
- **프로젝트 폴더에 발자국을 안 남긴다** — 어느 프로젝트를 열어도 작업 폴더엔 자동으로는 아무 파일도 안 생긴다. 지식은 `~/.claude/CLAUDE.md`의 관리블록이 `@import`로 주입한다. 관리블록은 플러그인이 만들고 갱신하는, 시작과 끝을 표시로 구분해 둔 구간이다. 자동 계층이 이미 있는 프로젝트 파일에 손대는 예외 둘은 아래 「무엇을·왜 자동화하나」에 적었다.
- **메인 세션과 실측한 두 서브에이전트 종류에 도달** — PC-레벨 주입이라 메인 세션과 `general-purpose`·`claude` 두 종류가 같은 원칙·오답노트를 보유한다(2026-07-28 실측). 빌트인 읽기 전용 종류(`Explore`·`Plan`)에는 실리지 않고, 사용자가 직접 정의한 서브에이전트는 한 종류도 재지 않았으니 도달을 전제하지 말고 정본 경로를 프롬프트에 적어 준다. 실측 표와 그 한계는 [DESIGN-NOTES](docs/DESIGN-NOTES.md)에 있다.
...
- 설정 홈에 `agent-principles.md`·`domains-index.md`·`solved_problems.md`가 있다.
- 같은 설정 홈의 `CLAUDE.md`에 `# BEGIN disciplined-coder`로 시작하는 관리블록이 있다.

**설정 홈을 `~/.claude`라고 넘겨짚지 않는다.** 스캐폴드는 그곳을 환경에 따라 다르게 잡으므로, 파일을 찾기 전에 어디인지부터 물어본다.
