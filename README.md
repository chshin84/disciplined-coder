...
**대상** — 팀 디시플린을 모든 프로젝트와 서브에이전트에 걸쳐 PC 전역으로 강제하되, 프로젝트 폴더는 (자동으로는) 더럽히고 싶지 않은 Claude Code 사용자. (원하면 `/add-pointer`로 프로젝트별 오답노트는 옵트인 추가.)

## Highlights
- **프로젝트 폴더 footprint zero (자동)** — 지식은 `~/.claude/CLAUDE.md` 관리블록이 `@import`로 주입한다. 어느 프로젝트를 열어도 작업 폴더엔 **자동으로는** 아무 파일도 안 생긴다(예외: 사용자가 명시 실행한 옵트인 `/add-pointer` — 프로젝트별 오답노트).
