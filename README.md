...
## 설치
파이썬이 두 곳에서 필요하다. 세션이 시작될 때 도는 마켓플레이스 자동 갱신과, 승인을 받고 도는 오답노트 쪼개기다. 없으면 그 단계를 건너뛰면서 사유를 알리고 나머지는 그대로 동작한다.
...
## 프로젝트 폴더에 생기는 파일
자동 계층이 프로젝트 파일을 고치는 예외는 둘이고, 둘 다 그 레포에 오답노트나 관리블록이 이미 있을 때만 걸린다(여기가 그 정본이다). 첫째, 그 레포에 `docs/solved_problems.md`가 이미 있고 그 머리말이 낡았으면 갈아끼운다. 둘째, 없앤 기능이 남긴 관리블록이 그 레포 `CLAUDE.md`에 있으면 걷어낸다. 둘 다 손대기 전에 사본을 먼저 만든다 — 사본을 못 떴을 때 무엇을 하는지는 `domain-docs`가 정본이다. 사본은 프로젝트에 두지 않고 전역 백업에 쌓는다.

여기에 자동 계층이 파일을 고치지 않고 알리기만 하는 것이 하나 더 있다. 그 오답노트가 아직 한 덩어리면 항목 수와 함께 알리기만 하고, 색인 파일과 본문 파일로 쪼개는 것은 세션이 사용자에게 물어 승인받은 뒤 `migrate-solved-log` 스킬로 한다. PC 전역 오답노트와 프로젝트 오답노트를 똑같이 다룬다 — 항목 수만큼 파일이 새로 생기는 것은 어느 쪽에서나 되돌리기 어렵기 때문이다.

**그 레포에 파일이 새로 생긴다. 쪼갤 때뿐이고, 그것만 사용자 승인을 받고 실행한다.** 항목마다 하나씩 `docs/solved_problems/` 아래에 놓이므로, 스무 개짜리 오답노트를 쪼개면 파일이 스무 개 생긴다. 그 파일들은 플러그인 것이 아니라 원래 그 오답노트에 있던 사용자의 글을 옮겨 담은 것이다. 다만 깃으로 관리하는 레포라면 **커밋되지 않은 새 파일로 잡히므로**, 쪼갠 뒤 `git status`가 갑자기 길어진다. 세션 시작 출력은 어느 오답노트가 아직 한 덩어리이고 항목이 몇인지까지만 알린다 — 쪼갠 뒤의 사본 경로는 알리지 않으므로, 되돌리려면 깃으로 되돌린다.

두 예외와 쪼개기 모두 그 파일 옆에 임시 파일이 잠깐 생겼다 사라지고, 쪼갤 때는 본문을 모을 임시 디렉터리(`docs/solved_problems.tmp.a1B2c3`)도 함께 생기며, 관리블록을 걷어낼 때는 잠금 디렉터리도 함께 생긴다. 잠금 디렉터리는 두 세션이 같은 파일을 동시에 고치지 못하게 막는 표식이다. 임시 파일이 필요한 것은 파일을 통째로 바꿔치기하는 방식이라 같은 폴더에 두어야 하기 때문이다(다른 곳에 두면 디스크가 다를 때 바꿔치기가 원자적이지 않다). 작업이 끝나면 지워지므로 남는 파일은 없지만, 도중에 강제로 끊기면 `CLAUDE.md.lock`이나 `CLAUDE.md.lock.gate`, `solved_problems.md.a1B2c3`, `docs/solved_problems.tmp.a1B2c3` 같은 파일과 디렉터리가 남을 수 있다. 그때는 지워도 된다. 다만 스캐폴드가 스스로 지우는 것은 관리 디렉터리 안에 남은 **빈 파일**뿐이다. 내용이 든 파일과 디렉터리는 매 세션 잔존을 알리기만 하고 지우지 않으며, 프로젝트 폴더에 남은 것과 설정 홈에 남은 잠금 파일은 아예 훑지 않는다. 그래서 남은 것은 사람이 지운다. 관리 디렉터리의 잠금은 10초 뒤에 빼앗기고 그 문지기는 30초까지 기다린다.
...
- **오답노트 개편** — 오답노트가 아직 안 쪼개진 옛 형식이면 세션이 시작될 때 개편을 권하는 안내가 뜬다. 개편은 걸음이 둘이고 **둘 다 사용자에게 묻고 한다.** 색인 파일과 본문 파일로 가르는 걸음은 항목 수만큼 파일이 새로 생겨 되돌리기 어렵고, 색인 줄을 지시사항 한 문장으로 다시 쓰는 걸음은 뜻을 옮기는 일이라 값이 항목 수에 비례한다. PC 전역 오답노트와 프로젝트 오답노트를 똑같이 다룬다 — 자동 계층은 어느 쪽도 쪼개지 않고 항목 수만 알리며, 두 걸음의 방법은 `migrate-solved-log` 스킬이 소유한다. 새로 만드는 오답노트는 처음부터 색인 형식이라 아무것도 안 걸린다.
