...
개발 대상(도메인)에 따라 "마땅히 그래야 하는 것"이 있다. `domains-index.md`(`~/.claude/CLAUDE.md`에 자동 주입되는 목차)가 도메인 목록과 "언제·어느 참고서"를 안내하고, 각 `skills/domain-*`가 상세(온디맨드)를 제공한다 — 설계/계획 시 명세에 반영, 개발 시 폴백. 통증이 확인된 도메인만 둔다(빈 스텁을 두지 않음 — 없는 것이 정직하다).
