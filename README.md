...
- **함께 쓰는 플러그인 설치 권유** — 매 세션, superpowers 와 카파시(Andrej Karpathy)의 코딩 지침 플러그인 `andrej-karpathy-skills` 가 이 PC에 있는지 보고 없는 것만 설치 명령으로 알린다. 대신 깔지는 않는다. 깔려 있으면 아무것도 나오지 않고, 안 깔기로 정했으면 `~/.claude/disciplined-coder/plugin-notice.skip` 에 그 이름을 한 줄 적으면 조용해진다. 정본의 「Karpathy guidelines」 절 넷이 카파시 플러그인에서 옮긴 것이고, 목적어를 코드에서 산출물로 일반화했다.
