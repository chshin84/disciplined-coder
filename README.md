...
- **빌트인 읽기 전용 종류(`Explore`·`Plan`)** — 원칙이 실리지 않는다. 그리고 자기 서브에이전트를 정의해 쓴다면 원칙이 실린다고 믿지 마라 — 우리가 실제로 잰 것은 네 종류뿐이라 사용자가 직접 정의한 서브에이전트는 재 보지 않았다. 그런 리뷰어를 쓸 때는 정본 경로를 프롬프트에 넣어라.
