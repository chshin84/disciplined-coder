...
## 프로젝트 폴더에 생기는 것
자동 계층이 프로젝트 파일에 손대는 예외는 셋이고, 모두 그 레포에 오답노트나 관리블록이 이미 있을 때만 걸린다(여기가 그 정본이다). 첫째, 그 레포에 `docs/solved_problems.md`가 이미 있고 그 머리말이 낡았으면 갈아끼운다. 둘째, 그 오답노트가 아직 한 덩어리면 본오답노트와 개별노트로 쪼갠다. 셋째, 없앤 기능이 남긴 관리블록이 그 레포 `CLAUDE.md`에 있으면 걷어낸다. 셋 다 손대기 전에 사본을 먼저 뜬다 — 사본을 못 떴을 때 무엇을 하는지는 `domain-docs`가 정본이다. 사본은 프로젝트가 아니라 전역 백업에 쌓는다.

**둘째 예외에서만 그 레포에 파일이 새로 생긴다.** 항목마다 하나씩 `docs/solved_problems/` 아래에 놓이므로, 스무 개짜리 오답노트를 쪼개면 파일이 스무 개 생긴다. 그 파일들은 플러그인 것이 아니라 원래 그 오답노트에 있던 사용자의 글을 옮겨 담은 것이다. 다만 깃으로 관리하는 레포라면 **커밋되지 않은 새 파일로 잡히므로**, 쪼갠 뒤 `git status`가 갑자기 길어진다. 무엇을 어디서 쪼갰고 사본이 어디 있는지는 세션 시작 출력에 적힌다.
...
- **오답노트 개편** — 오답노트가 아직 안 쪼개진 옛 형식이면 세션이 시작될 때 개편이 걸린다. 개편은 걸음이 둘인데 묻는 시점이 다르다. **본오답노트와 개별노트로 가르는 걸음은 묻지 않고 쪼갠다** — 기계가 하는 일이고 사본을 먼저 뜬 뒤에만 손대며, 형식을 정하는 주체가 플러그인이라 머리말 갱신과 같은 근거로 승인을 기다리지 않는다. **본오답노트의 줄을 지시사항 한 문장으로 다시 쓰는 걸음은 묻는다** — 뜻을 옮기는 일이라 값이 항목 수에 비례하고, 그 방법은 `migrate-solved-log` 스킬이 소유한다. 새로 만드는 오답노트는 처음부터 색인 형식이라 아무것도 안 걸린다.
