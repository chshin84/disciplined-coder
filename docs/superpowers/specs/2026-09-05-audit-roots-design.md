...
그 밖의 전제 셋이다. 이 설계의 바탕은 커밋 `04e2ef1`이다. 같은 날 다른 세션이 main 작업 트리에 미커밋 변경을 열 파일에 걸어 두었다. 정본에 「Extra 지침」 절을 넣고 SIMPLE·SURGICAL·TDD 조항이 그 절을 가리키게 하며 스킬 넷(domain-docs·project-doc-audit·domain-spec-review·lens-adversarial)의 조항 낱말을 바꾸는 변경이다. 이 설계는 그 절을 건드리지 않는다. 이 브랜치와 그 변경의 병합 순서는 사용자가 정한다(🔴). 정해지면 이 문단을 그 결정으로 바꾸고, 어느 쪽이 먼저든 뒤에 오는 쪽이 정본 「원칙」 목록(READ-FLOW 조항은 바뀐 세 조항의 이웃이다)과 스킬 넷의 충돌을 푼다. writing-korean의 근거 수치 출처는 사용자 결정으로 이 설계 밖이다.
...
- 다른 세션의 미커밋 변경이 이 설계가 고치는 스킬 넷과 정본을 함께 만진다. 병합 순서를 사용자가 정한 뒤 뒤에 오는 쪽이 충돌을 푼다(전제 절의 🔴).
