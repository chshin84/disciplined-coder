...
그 밖의 전제 셋이다. 이 설계의 바탕은 커밋 `04e2ef1`이다. 같은 날 다른 세션이 main 작업 트리에 미커밋 변경을 열 파일에 걸어 두었다. 정본에 「Extra 지침」 절을 넣고 SIMPLE·SURGICAL·TDD 조항이 그 절을 가리키게 하며 스킬 넷(domain-docs·project-doc-audit·domain-spec-review·lens-adversarial)의 조항 낱말을 바꾸는 변경이다. 이 설계는 그 절을 건드리지 않는다. 병합 순서는 사용자가 정했다. 다른 세션의 변경이 먼저 main에 들어가고 이 설계가 뒤에 간다. 그래서 plan의 0단계는 그 커밋 위로 이 브랜치를 옮기고(rebase) 2절이 가리키는 문장 위치를 다시 맞추는 것이며, 정본 「원칙」 목록(READ-FLOW 조항은 바뀐 세 조항의 이웃이다)과 스킬 넷의 충돌은 이쪽이 푼다. 구현 세션이 시작할 때 그 변경이 커밋되어 있지 않으면 구현을 시작하지 않고 사용자에게 알린다. writing-korean의 근거 수치 출처는 사용자 결정으로 이 설계 밖이다.
