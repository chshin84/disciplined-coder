...
> **제거된 기능의 설계다(superseded).** 여기서 설계한 처분 모드 토글은 뜯어냈고 지금은 설정할 수
> 없다. 처분을 어떻게 하는지는 `agent-principles.md`의 오답노트 절이 정본이다. 왜 토글로 가려 했고
> 왜 접었는지의 판단만 쓸모가 있어 남겨 두는 것이니, 지금 동작의 근거로 읽지 마라.
