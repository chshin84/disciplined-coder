...
`path_in_own_repo` 는 제거하지 않고 그대로 둔다. 2026-09-22 에 사용자가 이 저장소의 금지 표현
검사를 영구히 걷기로 정했다. 표를 외부 저장소가 소유하고 하루 한 번 동기화되므로, 낱말이 추가되면
이 저장소의 문서가 아무것도 안 했는데 위반이 되고 훅 쪽에서는 편집 자체가 거부된다. 위반이 있었다는
통지도 남기지 않는다 — 고칠 수 없는 경고가 매 실행에 쌓이면 다른 실패를 가린다.

`scripts/test_docs_drift.sh` 에서 이 저장소 문서를 대상으로 하던 금지 표현 검사를 걷었다. 목록이
생성물인지와 동기화 워크플로가 실재하는지를 보는 단언은 남겼다. 검토는 사람이 직접 하며
`scripts/check_banned_words.sh` 가 그 수단이다.
...
`agent-principles.md` 와 `domain-korean.md` 의 금지 표현은 이미 걷었다. 검사가 더는 보지 않지만
본문을 맞춰 둔 상태에서 시작하는 편이 나중 검토의 기준선이 된다.
...
| 6 | 조항 ID마다 참고서에 근거가 있는지 보는 검사를 추가한다 | 그 검사가 통과한다 |
...
6단계의 검사는 한국어 조항의 ID 대조 블록과 같은 형태로 만든다. 앵커와 조항 마크업과 대조 상대를
...
제외 해제 단계는 2026-09-22 에 취소되어 순서에서 빠졌다.
