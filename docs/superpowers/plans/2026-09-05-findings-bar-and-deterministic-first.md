...
| 워크트리 격리가 복합 셸 명령을 거부한다 | 여러 명령을 한 줄에 잇거나 변수를 명령 이름 자리에 두면 도구가 실행을 거절한다 | 명령을 나눠 실행하거나 스크립트 파일로 옮겨 부른다 |
...
넷의 「출력 스키마」 절 바로 앞에 아래 절을 넣는다. `<이 렌즈가 기계에 넘기는 것>` 자리에는 렌즈마다 다른 문장을 넣는다.
