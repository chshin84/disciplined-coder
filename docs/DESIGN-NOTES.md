...
## 서브에이전트로의 지식 전달
아래 내용은 Claude Code 공식 문서의 서브에이전트 메모리 로딩 규칙에 근거한다(2026-06 검증 —
이후 버전에서 바뀔 수 있으니 의심되면 공식 문서에서 재확인한다).
- **PC 전역 solved(오답노트)는 모든 커스텀 서브에이전트에 자동 주입된다.** 서브에이전트는 시작 시
...
`Stop` 훅은 git 신규(미추적·추가) spec/plan 중 마커 없는 게 있을 때만 작동하므로(`git status`로 거름; 기존 파일 수정은 제외 — Fix A), 설계 문서를
...
**Fix A 트레이드(인지)**: Stop 하드게이트는 **신규(미추적·추가) spec/plan에만** 건다 — 기존 커밋된 spec을
...
같은 턴에 작성+커밋된 spec도 클린이라 안 걸린다(기존 FAIL-OPEN). 필요하면 그 spec을 새 파일로 떠 리뷰하거나
게이트를 강화한다.
