...
# 소비자는 Bash 명령을 보는 훅이다. 모든 호출에 도는 거르기는 프로세스를 아끼려고
# _hook_input.sh 의 hook_command 로 셸 확장만 쓰고, 대상을 뽑을 때는 여기 awk 가 명령 전체를 푼다.
# grep 한 줄로 못 뽑는 이유는 명령에 큰따옴표가 들어가기 때문이다 — `"[^"]*"` 로 끊으면 첫 \" 에서
# 잘려 뒤가 통째로 사라지고, 그러면 그 뒤에 놓인 명령어를 훅이 못 본다.
