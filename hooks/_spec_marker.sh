...
# 영구로 정했고 위반이 있었다는 통지도 남기지 않기로 했다. 표를 외부 저장소가 소유하고 하루 한 번
# 동기화되므로, 낱말이 추가되면 이 저장소의 문서가 아무것도 안 했는데 편집이 거부된다. 표와 근거를
# 적는 문서는 그 말을 이름으로 불러야 한다는 것도 이유다. 검토는 사람이 직접 한다 —
