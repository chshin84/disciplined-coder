...
# 소비자는 Bash 로 고친 파일을 보는 훅이다. `_extract_path.sh` 가 Write·Edit 의 file_path 를,
# 이 파일이 Bash 명령의 대상을 맡는다. 명령 문자열을 꺼내는 것은 `_extract_command.sh` 가 한다.
#
...
#
# 값을 아끼려고 두 단계로 둔다. 쓰기 구문의 낱말이 아예 없으면 awk 를 안 띄우고 끝낸다. 세션의
# 대부분은 `ls`·`git status`·`grep` 이고 그것들이 여기서 바로 빠진다.
set -euo pipefail
