...
`SSOT`와 `NAME-ITEMS`와 `EXPLICIT`이 문서에도 그대로 걸리고, Simplicity First와 Surgical Changes와 Goal-Driven Execution의 목적어를 문서로 읽는다. spec과 plan을 쓰는 방법은 superpowers가 소유하므로 여기서 다루지 않고, 아래 표에는 그 둘의 수명과 표시 규칙만 담는다.
