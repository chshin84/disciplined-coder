...
- **꺼내 쓰기(recall)**: 디버깅·구현을 시작하기 전에 solved에서 비슷한 증상을 먼저 찾는다.
- **열린 것은 추적 말고 surface/위임**: 미해결은 **손유지 목록을 만들지 마라**(썩는다). 작업 중엔 메모리
  (머신로컬 작업노트)에 두거나 사용자에게 surface하고, must-keep는 **프로젝트의 진짜 트래커**(자동 close 있는
  GitHub Issues 등)로 위임한다.
