...
셋째 칸이 `문서와 답변`인 것은 저장소의 살아 있는 문서에도 걸리고 `scripts/test_docs_drift.sh`가 검사한다. `답변`인 것은 사용자에게 보내는 답에만 걸리고 `hooks/reply_check_stop.sh`가 검사한다. 첫째 칸의 백틱 안이 검색할 글자 그대로다.
...
| `꼴` | 형태 · 형식 | 답변 |
| `갈래` | 종류 · 분기 | 답변 |
| `회차` · `째 판` · `이번 판` | 무엇의 몇 번째인지를 이름과 함께 | 답변 |
| `낱말` | 단어 | 답변 |
| `품이 든` | 수고가 든다 · 손이 더 간다 | 답변 |
| `짚` | 지적한다 · 가리킨다 | 답변 |
| `훑` | 살펴본다 · 검토한다 | 답변 |
| `맞대` · `맞댄` | 대조한다 | 답변 |
