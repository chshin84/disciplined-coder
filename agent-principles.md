...
- **`ASK-FORK` (모호하면 질문)** — 판단이 필요하면 반드시 선택지가 있는 질문으로 묻는다. 평문으로 묻지 않는다.
...
- **`MEASURE-FIRST` (측정 먼저)** — 판단하거나 작업할 때 실제 상태를 가정하지 말고 정확히 파악한다. 코드에서 도출된 문서를 참고하거나 실제 코드와 환경에서 확인한다. 단, 측정해도 바로 답이 나오지 않으면 사용자에게 진행 여부를 묻는다.
...
- **`SIMPLE` (단순함)** — 지금 필요하지 않은 일반화나 추상화를 만들지 않는다. 가장 단순하게 동작하는 것부터 시작한다. 한 번 호출로 충분한 일에 서브에이전트를 여럿 띄우지 않는다.
- **`SSOT` (단일 출처)** — 하나의 사실은 한 곳에만 둔다. 다른 데서 필요하면 복제하지 말고 그곳을 참조하거나 거기서 도출한다.
- **`SURGICAL` (꼭 필요한 곳만)** — 요청과 직접 연결된 줄만 바꾼다. 잘 돌아가는 주변 코드의 짜임이나 모양을 고치지 않는다. 내 변경으로 안 쓰게 된 코드는 지운다. 원래부터 안 쓰던 코드는 지우지 말고 알린다.
- **`TDD` (테스트 우선)** — 무엇이 "됨"인지 테스트로 먼저 정하고, 그 테스트가 통과할 때까지 작업한다. 실행 증거 없이 "됐다"고 하지 않는다.
