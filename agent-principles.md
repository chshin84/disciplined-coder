...
공통 방법(PREP → 독립 렌즈 → 메타 집계 → accept/regenerate/escalate)의 상세·실행은 호출자 스킬이
SSOT다 — `domain-spec-review`(메타 산출물)·`domain-llm-runtime`(런타임). 골자만: 단독 LLM 결론으로
끝내지 말고 *안 쓴 신선한 눈*으로 본다(방법은 같고 대상만 다르다).

문서(README 등)는 작성 전 양식 제안 + 작성 후 비자가 검진을 훅이 자동으로 띄운다(Pre 제안·Post 넛지,
비블로킹). 셀프 퇴고만으론 부족하니 객관적 검진으로 보완하되, 발행물이라 마커는 안 박는다(spec/plan은
하드 게이트). 양식·렌즈 상세는 `domain-docs`·`reviewer-grounding`+`reviewer-fit` 참조.
