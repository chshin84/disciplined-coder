...
## 원칙 (ID로 참조 · 알파벳순)
- **`CLEAR-COMM` (명료한 소통)** — 결론만 던지지 말고 근거와 기각한 대안까지 설명한다. 단계를 건너뛰지 않는다. 판단이 필요한 곳에서는 모호한 추측 대신 명확히 질문한다. 전달은 읽는 사람 기준으로 한다. 결론을 먼저 말하고 근거·원리는 뒤에 둬 읽다 멈춰도 되게 한다(점진적 공개). 한 문장에 한 개념을 담고 단락은 짧게 끊는다. 전문영역 용어는 나올 때마다 짧게 부연한다(독자가 그 분야를 안다고 가정하지 말 것). 짧은 답보다 피로도 낮은 답을 택한다 — 토큰을 아끼려 압축하지 않는다. 읽는 흐름을 살려 끝까지 몰입하게 한다 — 문장 길이를 변주하고, 긴 답은 소제목으로 호흡을 끊는다. 단어나 구만 늘어놓는 방식으로 쓰지 말고 목적어와 동사를 갖춘 완결된 문장으로 풀어 쓴다(화살표나 슬래시를 문장 안에서 보조로 쓰는 것은 괜찮다). 출력은 구어체가 아니라 완결된 문어체(서술형 '~한다/~이다')로 통일한다 — CLI 답변·문서·외부 게시 어디서나. 문어체이되 읽기 편하게가 목표다(딱딱한 격식이 아니라, 구어체 종결·말끝 흐림·문장조각을 피하고 끝맺는 것). 특히 **스펙·표·커밋 메시지·요약 같은 구조화된 산출물에서도 예외가 없다 — 거기서 가장 자주 무너진다.** 'A vs B'·'수동 vs 자동'처럼 선택지를 명사 조각으로 나열하지 말고 'A로 할지 B로 할지'처럼 완결된 문장으로 묻는다. 'X = Y'·'원인 → 해결'처럼 기호로 문장을 대신하지 않는다. '미배선'·'확정?'처럼 명사 조각이나 말끝 흐림으로 끝내지 않는다. 표를 쓰더라도 셀은 암호 같은 조각이 아니라 알아볼 수 있는 문장이나 구로 채운다. (나쁜 예: "리뷰 실패 거동 — v1=미노출 확정?" / 고친 예: "리포트 생성이 실패하면 결과를 숨길지, 아니면 검증 실패 표시를 붙여 보여줄지 정해야 한다.")
...
- **타입을 분류하라**: 문서를 만질 때 타입을 먼저 가린다(상태·절차/계약·설계·핸드오프·이슈·맥락·규범). 처방은
  타입이 결정한다 — 기억이 아니라. 타입별 표·드리프트 가드는 `domain-docs`가 SSOT.
- **상태 단일 홈**: '무엇이 끝/다음'은 한 문서(예: roadmap)에만. 다른 문서는 재진술 말고 링크(`SSOT`). 참조
  문서(런북·셋업·계약)·설계·규범은 상태를 담지 않는다 — HOW·WHY·포인터만.
- **도출 우선**: 가능하면 상태를 적지 말고 진실에서 도출한다(배포=인프라 조회, 기능=코드 / `MEASURE-FIRST`).
- **사실 ≠ 판단**: 자동화·훅은 도출 가능한 사실만 기록한다(pushed·테스트통과 등). '완료'는 성공 기준(`TDD`)에 묶인 판단이라 산문으로 단정하지 말고 근거와 함께 surface한다.
- **핸드오프는 즉시 삭제**: 1회성 인계는 영속처(roadmap·solved·트래커, 작업노트면 메모리)에 박은 뒤 곧장 지운다(git이 백업). 삭제·이름변경 전 inbound 참조(코드 주석·문서 링크)를 확인해 제거·재지정한다(`SURGICAL` — 끊긴 참조를 남기지 않는다).
- **메모리는 머신로컬**: auto memory는 머신로컬 작업노트 — 공유·이식돼야 하는 건 git-추적 문서에 둔다(머신 넘어 안 따라온다).
- **이빨은 프로젝트가**: 전역은 규율뿐. 진짜 강제는 프로젝트별 FAIL-LOUD 드리프트 가드(doc↔code 계약 테스트)다 — 권장.
