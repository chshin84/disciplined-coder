...
description: Reviewer lens that reads a design document (spec or plan) against itself and against its paired document — internal contradictions, coverage gaps between spec and plan, name or type drift, and scope that runs too broad for one implementation plan and needs splitting, or has been sliced too thin and needs merging. Use it when the question is whether the document hangs together, rather than whether its content is true or where it breaks. domain-spec-review runs it as a read-only subagent.
---
