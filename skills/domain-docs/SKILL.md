...
스캐폴드가 "형식 규칙 서술이 현행과 다르다"고 알리면 **자동으로 고치지 않는다.** 사용자에게 권장안과
선택지를 주고 승인을 받아 메인 세션이 고친다. 방법은 넷이다.
- **`<관리 디렉터리>/backups/` 아래에 타임스탬프를 붙인 사본을 먼저 뜬다.** 그 로그는 git 밖이라 사본이
  유일한 복구 수단이다. 회전이나 상한을 두지 않는다 — 상한을 두면 첫 수정 직전 원본이 먼저 지워진다.
- **옛 형식 규칙 서술을 지우고 새 블록을 넣는다.** 새 블록만 끼워 넣으면 한 파일이 두 형식을 규정한다.
- **항목은 한 줄도 건드리지 않는다.** 항목은 사람이 쓴 산문이라 기계가 옮기면 뜻이 상한다.
- **무엇을 바꾸는지 보여 주고 승인을 받는다.** 애매하면 판단하지 말고 사용자에게 보인다.

신호는 고칠 때까지 세션마다 반복된다. 끄는 수단을 두지 않은 것은, 끄면 낡은 형식 정본이 신호 없이 남아
고치려던 상태로 되돌아가기 때문이다(`FAIL-LOUD`). 막지 않는 넛지이므로 반복을 감수한다.
