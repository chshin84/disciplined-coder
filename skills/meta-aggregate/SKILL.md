...
**`principles_applied`를 언제 요구하는지는 여기가 정본이다.** Claude 서브에이전트로 띄운 리뷰에서만 호출자가 이 값을 요구한다 — 그 종류에는 원칙 정본이 안 실릴 수 있어 읽고 적용했다는 흔적을 산출물에 남긴다. **제품 런타임 구현에는 요구하지 않는다.** 렌즈 파일은 이 규칙을 되풀이하지 않고 여기를 가리킨다.
