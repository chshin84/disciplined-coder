...
description: 멀티태스크 플랜이 둘 이상일 때 3층(오케스트레이터→서브오케스트레이터→워커·리뷰어)으로 병렬 실행하는 방법. 스펙별 독립 워크트리·자율 L2·기계적 소유권 강제·무상태 재개. agent-principles 병렬 오케스트레이션 절가 트리거.
---
# nested-orchestration — 3층 병렬 오케스트레이션 (방법 SSOT)

> `agent-principles.md` 병렬 오케스트레이션 절가 트리거 인덱스다. 여기가 *어떻게*의 SSOT다. 이 스킬은 기존 스킬을
