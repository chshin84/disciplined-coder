...
- 각 단위가 자기 계획·구현·리뷰 루프를 가진 덩어리인 **멀티태스크 플랜**이면 이 스킬(3층)을 쓴다.
...
2. **L2(자율 서브오케스트레이터, 사람 대화 불가)**: 잠긴 스펙으로 `writing-plans`(계획부터) →
   ...
   **검증은 세 지점에 배선한다** — 스펙 국면은 L1이 이미 `domain-spec-review`(훅 강제)로 마쳤고, **플랜 국면은
   ...
   BLOCKED로 버블업한다 — 🔴가 나왔을 때, 렌즈끼리 같은 지점을 두고 반대로 판정했을 때, 리스크상
...
4. **방식(TDD + 3층)** — 구현자가 구현하되 같은 파일은 순차로 편집하고 병렬 mutate는 금지하며, 프로젝트
   테스트 규약을 따른다. **읽기전용 리뷰어 서브에이전트**가 diff를 읽고 findings를 돌려주면 L2가 고친다(리뷰어는
