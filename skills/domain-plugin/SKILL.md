...
- **frontmatter** — 스킬 `SKILL.md`는 `name`(디렉터리 이름과 같게)과 `description` 두 키를 쓰고, 커맨드 `.md`는 `description` 하나를 쓴다. `description`은 언제 여는지를 포함한다. 이 요구는 스킬과 커맨드 양쪽에 적용된다. 스킬은 Claude 가 언제 열지 고르는 근거가 그 값뿐이고, 커맨드는 사용자가 목록에서 고르는 근거가 그 값뿐이다. 값은 YAML 평문 스칼라라 `: `와 ` #`을 넣지 않는다.
...
- **스킬 이름의 부류** — 이름의 모양이 그 스킬을 어떻게 쓰는지 말하게 한다. 열어서 읽는 규칙집에는 `domain-` 접두사를 붙이고(`domain-readme`·`domain-plugin`), 읽기 전용 서브에이전트로 실행하는 렌즈에는 `lens-` 접두사를 붙이며, 렌즈를 실행해 결과를 내는 절차에는 접두사 대신 동사구를 쓴다(`review-specs`·`review-docs`·`review-llm-calls`·`audit-repo-docs`·`dispatching-lenses`·`aggregating-lenses`·`nested-orchestration`). 규칙집과 절차가 같은 접두사를 쓰면 여는 쪽이 무엇을 기대할지 알 수 없다.
- **컴포넌트 위치** — `agents/`·`skills/`·`commands/`·`hooks/hooks.json`에 둔다. 플러그인 루트의 CLAUDE.md는 컨텍스트로 로드되지 않는다.

## 훅을 만들 때의 값

훅은 값이 든다. 자주 실행되는 훅을 만들기 전에 그 값을 먼저 측정한다. 아래는 2026-09-07에 이 PC(윈도우 11, Git Bash)에서 측정한 것이고, 다른 환경에서는 다시 측정한다.
...
- **프로세스 수를 센다** — 셸 반복이 파이썬보다 싸다고 가정하지 않는다. 표의 행마다 파이프를 실행한 훅이 13,057밀리초였고, 같은 일을 파이썬 한 번으로 하니 1,108밀리초였다. 윈도우는 프로세스를 실행하는 값이 커서 반복이 그대로 값이 된다.
- **고정 문자열 검색은 로케일을 `C`로 둔다** — `C.UTF-8`에서 `grep`이 207밀리초, `awk`가 213밀리초 들던 것이 크게 줄었다. `grep -F`는 찾는 글자를 그대로 비교하므로 바이트로 비교해도 UTF-8 결과가 같다.
- **답마다 실행되는 훅은 만들지 않는다** — Stop 훅은 방금 쓴 답을 인자로 받지 않는다. 대화 기록 파일을 열고 JSON을 파싱해 마지막 답을 골라내야 하고, 그래서 파이썬을 실행하게 된다. 실제 대화 기록에서 답 하나에 1,021밀리초가 들어 그 훅을 걷어냈다. 답에 적용하는 규칙은 에이전트원칙에 적어 지시로 맡긴다.
...
- **사유를 구분한 통지** — 못 고친 차수는 까닭을 구분해 알린다. 설정을 못 읽은 것과 파이썬이 없는 것과 쓰기 권한이 없는 것은 사용자가 할 일이 서로 다르다.
- **처리기 하나** — JSON을 다루는 처리기를 여러 개 두지 않는다. 폴백을 하나 더 두면 한 번도 실행되지 않는 사본이 생기고, 실행해 보지 않은 코드에는 오류가 숨는다(에이전트원칙의 `YAGNI`).
- **서식 변경 고지** — 값을 채우면 파일 전체가 다시 기록되어 서식이 바뀔 수 있다. 바뀐 경로를 알리고 사본 위치를 함께 보인다.
