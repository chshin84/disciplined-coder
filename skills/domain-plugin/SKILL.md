...
- **답마다 도는 훅은 만들지 않는다** — Stop 훅은 방금 쓴 답을 인자로 받지 않는다. 대화 기록 파일을 열고 JSON을 파싱해 마지막 답을 골라내야 하고, 그래서 파이썬을 띄우게 된다. 실제 대화 기록에서 답 하나에 1,021밀리초가 들어 그 훅을 걷어냈다. 답에 거는 규칙은 에이전트원칙에 적어 지시로 맡긴다.
...
- **처리기 하나** — JSON을 다루는 처리기를 여러 개 두지 않는다. 폴백을 하나 더 두면 한 번도 안 도는 사본이 생기고, 안 돌아 본 코드에는 오류가 숨는다(에이전트원칙의 Simplicity First).
