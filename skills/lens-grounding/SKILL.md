---
name: lens-grounding
description: LLM 출력·주장이 그 출처(런타임=요청+맥락 / spec리뷰=검토 문서+주입된 사실)에 근거하는지 보는 렌즈다. 누락과 모순과 환각을 찾는다. 호출자(review-llm-calls의 런타임 리뷰, review-specs, review-docs, audit-repo-docs의 레포 문서 감사)가 source를 제공한다.
---
...
문서 검진과 레포 문서 감사에서 출처는 둘이다. 그 문서가 주장하는 대상 코드와 인프라, 그리고 그 문서가 이름으로 부른 소유자 문서다. 호출자가 그 경로를 다 주지는 못한다. 검진과 감사의 호출자는 정본 경로와 목적 한 줄과 계약 경로만 넣도록 되어 있어, 소유자 문서와 대상 코드의 경로는 프롬프트에 안 들어오는 것이 보통이다. 그때는 기다리지 말고 렌즈가 직접 열어 대조하고 연 것을 `read`에 적는다. 아래 「읽기 범위」가 이미 문서 밖을 반드시 읽으라고 정하므로 이것은 그 범위 안이다.
