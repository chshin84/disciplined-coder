...
**물을 때는 평문에 섞지 말고 질문 도구로 선택지를 띄운다**(`AskUserQuestion` 같은 것). 보고 끝에 한
문장으로 붙이면 읽는 사람이 답할 자리인지 아닌지 가리기 어렵고, 긴 보고의 마지막 줄은 그냥 지나치기
쉽다. 이건 리뷰를 한 바퀴 더 돌릴지를 정하는 갈림길이라 대답이 없으면 다음 걸음이 정해지지 않는다.
그런 도구가 없는 환경이면 평문으로 묻되 보고 본문과 눈에 띄게 갈라 놓는다.
