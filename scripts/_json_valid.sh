...
# 처리기는 파이썬 하나다. 인터프리터 이름만 고른다. 처리기를 둘 이상 두면 한 번도 안 도는 사본이
# 생기고 안 돌아 본 코드에는 오류가 숨는다(domain-plugin 「처리기 하나」). 둘 다 없으면 조용히
# 통과시키지 않고 실패로 계상한다(검증 불능은 통과가 아니다).
# 고르는 순서는 운영체제로 가른다. 윈도우는 python 이 먼저다 — python3 이 스토어 안내판이면 실패하는
# 데만 프로세스 하나를 쓴다. 맥·리눅스는 python3 이 먼저다 — python 이 없거나 파이썬 2 일 수 있다.
# 고른 이름은 _JSON_PY 에 두어 한 프로세스에서 한 번만 찾는다. 명령 치환 `$( )` 안에서 처음 고르면
# 그 서브셸에서만 남으므로, 여러 번 부르는 스크립트는 바깥에서 이 함수를 한 번 먼저 부른다.
_json_python() {  # _JSON_PY 에 쓸 수 있는 인터프리터 이름을 담는다. 없으면 1.
  [ -n "${_JSON_PY:-}" ] && return 0
  local c order="python3 python"
  case "${OSTYPE:-}" in msys*|cygwin*) order="python python3" ;; esac
  for c in $order; do
    if "$c" -c 'import sys' >/dev/null 2>&1; then _JSON_PY="$c"; return 0; fi
  done
  return 1
}
...
# UnicodeDecodeError 가 난다. 표준 입력으로 한국어를 파이프해도 같은 이유로 깨진다. 저장소의 모든 파이썬 호출이 json_run 하나를 지나므로 여기서 한 번
...
json_run() {  # $1=파이썬 프로그램 문자열, 나머지=그 프로그램의 인자. stdin은 그대로 넘긴다.
  local prog="$1"; shift
  _json_python || { echo "  (json_run: python3/python 모두 없음 — 검증 불능은 FAIL로 계상)" >&2; return 1; }
  PYTHONUTF8=1 "$_JSON_PY" -c "$prog" "$@"
}
