...
#
# 함께 들어가는 것을 막는 장치가 둘이고, 하나만으로는 여럿이 함께 들어간다.
...
#   시각을 락 안에 적어 두고, 기다리는 쪽은 그 시각을 읽어 나이를 확인한다. 내가 기다린 시간으로 세면
#   10초를 채운 쪽이 그새 새로 들어온 사람의 락까지 빼앗는다 — 락은 바뀌었는데 내 시계만 계속
#   돌기 때문이다.
# 잡은 시각이 없는 락은 표를 남기기 전에 죽었거나 옛 판본이 남긴 것이니 빼앗을 대상으로 본다.
#
# 문지기(`$lock.gate`)를 따로 두는 이유는 나이를 확인하는 것과 빼앗는 것이 갈라져 있으면 그 사이에
# 남이 새로 잡기 때문이다. 재고 빼앗고 잡는 세 걸음을 문지기 안에 함께 넣어 한 번에 하나만
# 하도록 만든다. 문지기는 마이크로초만 잡으므로 오래 잡혀 있으면 잡은 쪽이 죽은 것이고, 그때는
# 30초를 기다린 뒤 치운다.
# 락 나이 상한(초). 락을 잡은 쪽은 awk 두 번과 mv 한 번, 덧붙이기 한 번만 하고 푸므로 이보다 오래
# 잡힌 락은 주인이 죽은 것으로 본다. 10 이라는 값 자체는 경험값이다.
...
# 못 뜨면 아예 걷어내지 않는다(호출자가 기억하게 두지 않으려고 함수 안에 둔다).
