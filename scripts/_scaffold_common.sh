#!/usr/bin/env bash
# scaffold.sh에서 분리한 관리 디렉터리 정책(SSOT) — 여기가 에이전트원칙.

# 관리 디렉터리에 두는 에이전트원칙 파일. 두 스캐폴드가 복사하고 주입하는 것이 이 목록이다.
SCAFFOLD_FILES="agent-principles.md korean-banned-words.md"
# 화이트리스트는 에이전트원칙 파일에 backups 디렉터리와 사용자가 쓰는 파일을 더한 것이다. 위생 검사가 이
